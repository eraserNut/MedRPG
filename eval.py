...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--output_dir', default='./outputs/MS_CXR_r50',
    ...
    parser.add_argument('--eval_model', default='./outputs/MS_CXR_r50/best_checkpoint.pth', type=str)
    ...
    if utils.is_main_process():
        ...
        if args.dataset in ['MS_CXR', 'ChestXray8']:
            log_stats = {'test_model:': args.eval_model,
                        '%s_set_total_accuracy'%args.eval_set: round(results['total_accuracy'], 4),
                        '%s_set_total_miou' % args.eval_set: round(results['total_iou'], 4),
                        '%s_set_sub_accu' % args.eval_set: [round(j, 4) for j in results['category_accu']],
                        '%s_set_sub_iou' % args.eval_set: [round(j, 4) for j in results['category_iou']],
                        }
        else:
            log_stats = {'test_model:': args.eval_model,
                        '%s_set_total_accuracy'%args.eval_set: round(results['total_accuracy'], 4),
